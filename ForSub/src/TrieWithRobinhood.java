        ...
        public class TrieNode{
            ...
            public void prothema(TrieNode CheckPoint, String proth){   
                ...
                if (CheckPoint.wordLength!=0)
                		System.out.println("prothema:"+proth);
                for(int i=0;i<CheckPoint.size;i++)
                {
                	if(CheckPoint.array[i]!=null) 
                	{
               CheckPoint.array[i].prothema(CheckPoint.array[i],proth+(char)((CheckPoint.array[i].element.data)+'a'));
                	}
                }
            }
            ...
            public void prothemaWithTolerance(String word, int i, String proth, int misses)
            {
            	...
            	{
            		if(this.array[index]!=null&&this.array[index].element.data!=word.charAt(i)-'a')
            			misses++;
            		
            		if(this.array[index]!=null&&misses<=2)
            		{
            			this.array[index].prothemaWithTolerance(word,i+1,proth+(char)(this.array[index].element.data+'a'), misses);
            		}
            	}
            ...
            {
            	if (proth.length() > (word.length() + 2) || miss > 2) {
                    return;
                }
               if(i==word.length())
               {
            	   if(miss<=2&&this.wordLength!=0&&!proth.equals(word))
            	   {
            		   System.out.println(proth);
            	   }
            	   return;
               ...
               for(int index=0;index<this.array.length;index++)
               {
            	   ...
            	   if(this.array[index]!=null&&miss<=2)
            	   {
            		   if(change)
            		   this.array[index].prothemataWithBiggerSize(word,i+1,proth+(char)(this.array[index].element.data+'a'),miss,change);
            		  
            		   else 
            		   {
                		   this.array[index].prothemataWithBiggerSize(word,i,proth+(char)(this.array[index].element.data+'a'),miss,change);
            		   }
            	   }
               }
            }
...
/*                trie.insert("cat");
                ...
                */
                ...
                String[] testWords = { "cat", "bat", "rat", "bath", "castle", "rattles", "bathtub","plan" };
